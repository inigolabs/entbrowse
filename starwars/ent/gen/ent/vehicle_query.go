...
func (vq *VehicleQuery) QueryAppearedIn() *FilmQuery {
	query := &FilmQuery{config: vq.config}
...
func (vq *VehicleQuery) QueryPilotedBy() *PersonQuery {
	query := &PersonQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VehicleQuery) Clone() *VehicleQuery {
	...
	return &VehicleQuery{
		...
		withAppearedIn: vq.withAppearedIn.Clone(),
		withPilotedBy:  vq.withPilotedBy.Clone(),
...
func (vq *VehicleQuery) WithAppearedIn(opts ...func(*FilmQuery)) *VehicleQuery {
	query := &FilmQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
	vq.withAppearedIn = query
	return vq
}
...
func (vq *VehicleQuery) WithPilotedBy(opts ...func(*PersonQuery)) *VehicleQuery {
	query := &PersonQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
	vq.withPilotedBy = query
	return vq
}
...
//		GroupBy(vehicle.FieldCargoCapacity).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Vehicle.Query().
//		Select(vehicle.FieldCargoCapacity).
//		Scan(ctx, &v)
//
...
func (vq *VehicleQuery) sqlAll(ctx context.Context) ([]*Vehicle, error) {
	var (
		...
		_spec       = vq.querySpec()
		loadedTypes = [2]bool{
			vq.withAppearedIn != nil,
			vq.withPilotedBy != nil,
		}
	)
	...
	if query := vq.withAppearedIn; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Vehicle, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(vehicle.AppearedInPrimaryKey[1], fks...))
			},
	...
	if query := vq.withPilotedBy; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Vehicle, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(vehicle.PilotedByPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
